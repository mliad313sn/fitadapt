...
function regenerate(tool: string, context: CoachContext, change: 'time' | 'deload' | 'pain', patch: Partial<GenerateSessionInput>, env: ToolEnv, done: CoachReplyPart[], code: string): ToolExecution {
  ...
  let result: ReturnType<typeof generateSession>;
  try {
    result = generateSession(input, ctxAt(env));
  } catch {
    // FIX-A (SAF-3): the engine fails closed on an input without its required safety facts; the coach refuses.
    return invalid(tool, 'coach.tool.invalid_input');
  }
