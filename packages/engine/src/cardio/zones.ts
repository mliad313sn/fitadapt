...
/** Whole years of age on a date (null without a date of birth). */
export function ageOn(birthDate: CalendarDateValue | null | undefined, nowMs: number): number | null {
...
export function zonesFor(facts: ZoneFacts): HrZoneSet {
  const age = ageOn(facts.birthDate, facts.nowMs);
