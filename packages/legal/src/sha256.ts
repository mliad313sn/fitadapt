/**
 * SHA-256 (FIPS 180-4) in pure TypeScript, so the same hash is computed on the
 * device (Hermes has no node:crypto) and on the server. Used for content
 * hashes of legal texts and for the defensibility log's hash chain. Checked
 * against node:crypto in the tests.
 */
...
/**
 * Canonical JSON: object keys sorted, no whitespace, `undefined` properties
 * dropped. The same value always hashes to the same string on every platform.
 */
export function canonicalJson(value: unknown): string {
  if (value === null || typeof value !== 'object') {
    if (typeof value === 'number' && !Number.isFinite(value)) throw new Error('canonicalJson: non-finite number');
    if (value === undefined) throw new Error('canonicalJson: undefined');
