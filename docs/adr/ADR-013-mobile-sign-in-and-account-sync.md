...
- ~~Safety events written after a screening push run outside the sync transaction; if logging fails after the change is applied, the replay returns `duplicate` and the event is not written again (known limit; M19 should make the log write transactional with the store).~~ **Fixed in `3bff4b8`:** `SyncServer` calls its `onApplied` listener inside the store transaction and passes the transaction handle (`ServerStore<TTx>`, `PgServerTx.db`); the screening's safety events are appended in that PostgreSQL transaction (ADR-009 "same transaction" rule). A failed log write rolls back the change and its idempotency record and fails the push (HTTP 500); the device keeps the mutation in its outbox and retries. Covered by an integration test that injects the failure in the database.
