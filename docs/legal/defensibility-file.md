...
| `coach.tool_call` | M11: every tool call of the AI coach (valid or not), written with the stored conversation turn in one transaction | tool (or `unknown` for a name outside the tool list), outcome (applied, proposed, refused, invalid), reason code, engine version; never the input or any text |
