...
-- M11: messages and tool-call audits are append-only (ADR-031); rows leave only with the conversation
-- (the user deletes it, the ai_coach consent is withdrawn, the retention purge, or the account is deleted).
-- Renumbered at integration from M11's 0009_m11_coach (0009…0012 are the fix wave's).
