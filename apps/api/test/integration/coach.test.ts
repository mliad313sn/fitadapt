...
import { coachConversations, coachMessages, coachToolCalls, safetyLocks, syncChanges } from '../../src/db/schema.js';
import { PEPPER, bearer, createHarness, device, signIn, truncateAll, uniqueEmail, type Harness } from './harness.js';

/**
 * M11 on the server (ADR-031): the coach endpoint proxies the model
...
function todayInput(r: Ready, over: Partial<GenerateSessionInput> = {}): GenerateSessionInput {
  ...
  return { safetyProfile: r.safetyProfile, equipment: GYM, equipmentLoads: defaultEquipmentLoads('gym'), equipmentProfileId: r.gymId, minutesAvailable: 45, capacity: r.capacity, programSession: programSessionContext(day, day.sessions[0]!), history: [], jointFlags: {}, recentLoads: [], localDate: null, birthDate: BIRTH, experience: 'intermediate', intensityLock: { locked: false, since: null }, ...over };
}
...
describe('M11 red flag in chat → the M05 stop flow (goal condition 4)', () => {
  it('"chest pain" ends the session, logs S3 in the sync transaction, and sessions stay refused until a review is attested — without a model call', async () => {
    ...
    // API-5 (FIX-C): synced session times are bounded by the server clock, so the session is dated by it (not MON):
    // the refusal asserted is the S3 lock, not a clock refusal.
    const at = h.clock.now().getTime();
    const g = generateSession(todayInput(r), createEngineContext({ clock: fixedClock(at), seed: 7 }));
    if (g.status !== 'ok') throw new Error('no plan');
    const record: WorkoutSessionRecord = { schemaVersion: 1, input: todayInput(r) as WorkoutSessionRecord['input'], plan: g.plan, safetyEvents: [...g.safetyEvents], startedAt: new Date(at + 60_000).toISOString(), jurisdiction: 'GB', firstWorkout: true };
  ...
  it('integration FIX-C × M11 (MOB-08): the S3 lock the server retains apart from the logs also stops every coach rebuild; the chain with coach.tool_call verifies against its anchored head', async () => {
    const r = await ready();
    // Only the retained lock fact (as after a health-consent withdrawal erased the red flag's execution log).
    await h.database.db.insert(safetyLocks).values({ userId: r.s.userId, recordId: randomUUID(), kind: 'red_flag', at: new Date(MON - 3_600_000).toISOString(), flagId: randomUUID() });
    switchable.inner = scriptedModel([{ toolCalls: [{ name: 'adjustSessionTime', input: { minutes: 30 } }] }, { text: 'Training is paused.' }]);
    const { conversationId } = (await start(r.s)).json() as { conversationId: string };
    const { body } = await say(r.s, conversationId, 'Can I just do 30 minutes?', context(r));
    expect(body.toolCalls[0]).toMatchObject({ tool: 'adjustSessionTime', status: 'refused', reasonCode: 'session.unavailable.s3_intensity_locked' });
    expect(body.actions).toEqual([]);
    // FIX-E (PKG-01): the coach.tool_call entry is in a chain that verifies against its anchored head.
    expect(await coachEvents(r.s)).toEqual([{ tool: 'adjustSessionTime', status: 'refused', reasonCode: 'session.unavailable.s3_intensity_locked', engineVersion: ENGINE_VERSION }]);
    const ref = h.app.services.legal.subjectRef(r.s.userId);
    expect(await h.app.services.legal.log.verify(ref)).toMatchObject({ ok: true });
  });

  it('integration FIX-A/FIX-E × M11: a context without the required safety facts is refused at the boundary (no engine call, no audit)', async () => {
    const r = await ready();
    switchable.inner = scriptedModel([{ toolCalls: [{ name: 'adjustSessionTime', input: { minutes: 30 } }] }, { text: 'ok' }]);
    const { conversationId } = (await start(r.s)).json() as { conversationId: string };
    const ctx = context(r);
    const { jointFlags: _drop, ...partial } = ctx.today!.input;
    const res = await h.app.inject({ method: 'POST', url: `/v1/coach/conversations/${conversationId}/messages`, headers: bearer(r.s.token), payload: { text: 'Can I just do 30 minutes?', context: { ...ctx, today: { ...ctx.today!, input: partial } } } });
    expect(res.statusCode).toBe(400);
    expect(await h.database.db.select().from(coachToolCalls).where(eq(coachToolCalls.conversationId, conversationId))).toEqual([]);
  });
