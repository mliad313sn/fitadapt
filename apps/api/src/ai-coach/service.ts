...
/**
 * The AI coach behind the API (M11; ADR-031). Server-side model proxy with
...
export class CoachService {
  ...
  async hardenContext(userId: string, context: CoachContext): Promise<CoachContext> {
    ...
    if (today) {
      ...
      const fromLogs = intensityLockStatus(events);
      // MOB-08 (ADR-027): and the lock the server retains apart from the erasable logs (it outlives a consent withdrawal).
      const retained = fromLogs.locked ? null : await retainedIntensityLock(this.deps.db, userId);
      const storedLock = fromLogs.locked || !retained?.locked ? fromLogs : { locked: true, since: retained.since };
      const input = {
        ...today.input,
        safetyProfile,
