...
import type { NutritionSettings } from '../nutrition/nutrition-store';
...
function SetupForm({ onDone }: { onDone: () => void }) {
  ...
  const save = () => {
    ...
    const previous = state.plans[state.plans.length - 1]?.data ?? null;
    ...
    const reason = previous ? 'settings_changed' : 'setup';
