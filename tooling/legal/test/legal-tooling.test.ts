...
describe('pnpm legal:licences (L6)', () => {
  ...
  it('validates the asset/dataset licence register of the repository', () => {
    ...
    // M06 added DATA-003 (exercise library seed): 4 → 5 entries; M03 added AST-003 (spoken cues: the system voice, no sound file): 5 → 6; M10 added DATA-004 (original food seed): 6 → 7. Still an exact count.
    expect(checkAssetRegister(md, [], () => true, policy)).toEqual({ problems: [], entries: 7 });
  });
